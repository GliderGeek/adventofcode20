function parseLine(line: string){
    let result = line.match(/([a-z]+ [a-z]+) bags contain (.*)/)
    let color = result[1]
    let rest = result[2]
    let rule = {
        "color": color,
    ...
    let contents = rest.match(/([0-9]) ([a-z]+ [a-z]+) bags?/g)
    if (contents != null){
        contents.forEach(mainMatch => {
            let subMatch = mainMatch.match(/([0-9]) ([a-z]+ [a-z]+) bags?/)
            let amount = parseInt(subMatch[1])
            color = subMatch[2]
            rule["contents"][color] = amount            
        });
    }
...
function getRules(lines: string[]): object[]{
    let rules = []
    lines.forEach(line => {
...
function part1(input: string[]): number{
    ...
    while (targets.length > 0){
        ...
        rules.forEach(rule => {
            if (target in rule["contents"]){
                let colorFound = rule["color"]
                if (!(colorFound in found)){
                    found[colorFound] = null
                    targets.push(rule["color"])
                }
            }
        });
    }
...
function part2(input: string[]): number{
    function getAmount(rules: object[], target: string): number{
        let total = 0
        rules.forEach(rule => {
            if (rule["color"] == target){
                for (let color in rule["contents"]){
                    let amount = rule['contents'][color]
                    total += (amount + amount * getAmount(rules, color))
                }
    ...
    let rules = getRules(input)
    return getAmount(rules, "shiny gold")
}
