function part1(entries:string[]):number{
    ...
    for (var i:number=0; i<entries.length; i++){
        ...
        let matches = entries[i].match(/(\d*)-(\d*) (.): (.*)/)
        
        let min_occurences: number = parseInt(matches[1])
        let max_occurences: number = parseInt(matches[2])
        let character: string = matches[3]
        let text: string = matches[4]

        console.log(min_occurences, max_occurences, character, text)
...
function part2(entries:string[]):number{
    let validPasswords: number = 0
    for (var i:number=0; i<entries.length; i++){

        let matches = entries[i].match(/(\d*)-(\d*) (.): (.*)/)
        
        let first_position: number = parseInt(matches[1]) - 1
        let second_position: number = parseInt(matches[2]) - 1
        let character: string = matches[3]
        let text: string = matches[4]

        if ((text.charAt(first_position) == character && text.charAt(second_position) != character) ||
            (text.charAt(first_position) != character && text.charAt(second_position) == character)){
                validPasswords++
            }
    }
...
var result:number = part1(entries)
console.log("Result part1:", result)

var result: number = part2(entries)
