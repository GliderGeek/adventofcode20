//number of questions at least one person answers yes to
function countYesOnce(group: string[]): number{
    let questions = {}
    for (let i = 0; i < group.length; i++) {
        const person = group[i];
        
        for (let j = 0; j < person.length; j++) {
            const letter = person[j];

            if (!(letter in questions)){
                questions[letter] = null
            }
        }
    }
...
//number of questions all persons answers yes to
function countYesAll(group: string[]): number{
    let questions = {}
    ...
    for (const letter in questions){
        if (questions[letter] == group.length){
            allYes++
        }
    }
